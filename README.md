...
- I added two new properties to `config.json/dataset_properties` (min_freq and fixed_length) to reduce the memory consumption. You are still able to use dynamic input size and assign every seen word in your vocabulary if you have enough memory.
- Sadly, I encountered the worst problem in PyTorch related to CUDA OOM error, which is model reloading increases the memory consumption =/ In short, I could start a training process (English dataset/non-static zeroes/text_cnn) and it iterated for 2 epochs without a problem. Then, I saved the model to continue the process later. However, after I reloaded the model, the code directly raised CUDA OOM error. I tried to apply things that I've read in PyTorch's forums; however, those so called fixes did not help me. Things that I've found and tried:
